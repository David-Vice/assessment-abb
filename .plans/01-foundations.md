...
> - **Schema as built (P3 refinement):** the live `init.sql` uses `embedding HALFVEC(3072)` with `hnsw (embedding halfvec_cosine_ops)` (pgvector caps `vector` HNSW at 2000 dims; `halfvec` reaches 4000 at half the storage), an **`IMMUTABLE` `immutable_unaccent()`** wrapper so `unaccent` is usable in a generated column, and `tsv` as a **`GENERATED ALWAYS … STORED`** column (`to_tsvector('simple', immutable_unaccent(content))`) plus `pg_trgm` GIN on `immutable_unaccent(content)`. Equal-language design unchanged.
